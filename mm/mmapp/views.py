        ...
        try:
            ...
            if Note.objects.filter(file_name=file.name,
                ...
                if len(oov) != 0:
                    ...
                    train_model(notebook_vocab, notebook_oov, notebook.kv, notebook.kv_vectors, schedule=timezone.localtime())
...
def merge_notebooks(request):
    ...
    if request.method == 'POST':
        ...
        if len(notebook_oov) != 0:
            ...
            train_model(notebook_vocab, notebook_oov, new_notebook.kv, new_notebook.kv_vectors, schedule=timezone.localtime())
        else:
            print("no oov, no need for training")
